...
void SetupAgent::configure(const pugi::xml_node& node, const std::string& configurationPath) {
	...
	if (!(att = node.attribute("setupTime")).empty()) {
		m_setupTime = std::stod(simulation()->parameters().processString(att.as_string()));
	}
	...
	if (!(att = node.attribute("bidPrice")).empty()) {
		m_bidPrice = std::stod(simulation()->parameters().processString(att.as_string()));
	}

	if (!(att = node.attribute("askPrice")).empty()) {
		m_askPrice = std::stod(simulation()->parameters().processString(att.as_string()));
	}
}
...
void SetupAgent::receiveMessage(const MessagePtr& msg) {
	...
	if (msg->type == "EVENT_SIMULATION_START") {
		auto bidPayload = std::make_shared<PlaceOrderLimitPayload>(OrderDirection::Buy, m_bidVolume, Money(m_bidPrice));
		auto askPayload = std::make_shared<PlaceOrderLimitPayload>(OrderDirection::Sell, m_askVolume, Money(m_askPrice));
