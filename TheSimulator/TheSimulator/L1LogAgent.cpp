...
void L1LogAgent::receiveMessage(const MessagePtr& messagePtr) {
	...
	// std::cout << messagePtr->type << std::endl;
	...
	if (messagePtr->type == "EVENT_SIMULATION_START") {
		if(!m_aggregationPeriod) {
			...
			logHead();
			simulation()->dispatchMessage(currentTimestamp, 1, name(), m_exchange, "RETRIEVE_L1", std::make_shared<EmptyPayload>());
		} else {
		...
		// std::cout << "occurrence: " << messagePtr->occurrence << std::endl;
		// std::cout << "arrival: " << messagePtr->arrival << std::endl;
...
Timestamp L1LogAgent::computeNextAggregation(Timestamp current) const {
	...
	Timestamp nextAggregationDelta = fmod(current, m_aggregationPeriod);
...
void L1LogAgent::logHead() {
	m_outputFile << "time,bestBidPrice,bestAskPrice,bestBidVolume,bestAskVolume" << std::endl;
}

void L1LogAgent::logData(std::shared_ptr<RetrieveL1ResponsePayload> pptr) {
	m_outputFile << std::to_string(pptr->time) << "," << pptr->bestBidPrice.toCentString() << "," << pptr->bestAskPrice.toCentString() << "," << std::to_string(pptr->bestBidVolume) << "," << std::to_string(pptr->bestAskVolume) << std::endl;
	...
	// std::cout << std::to_string(pptr->time) << ": BID " << pptr->bestBidPrice.toCentString() << " ASK " << pptr->bestAskPrice.toCentString() << " BID VOLUME " << pptr->bestAskVolume << " ASK VOLUME " << pptr->bestBidVolume << std::endl;
}
