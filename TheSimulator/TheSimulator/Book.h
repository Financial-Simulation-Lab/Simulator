...
public:
	...
	std::string printCSV(unsigned int depth) const;
...
private:
	...
	template <class CIteratorType>
	std::string dumpCSVLOB(CIteratorType begin, CIteratorType end, unsigned int depth) const;
};
...
template<class CIteratorType>
std::string Book::dumpCSVLOB(CIteratorType begin, CIteratorType end, unsigned int depth) const {
	while (depth > 0 && begin != end) {
		const Volume totalVolume = std::accumulate(begin->begin(), begin->end(), (Volume)0, [](Volume acc, const LimitOrderPtr& iop) {
			return acc + iop->volume();
		});
		...
		return "," + begin->price().toPostfixedString(3) + "," + std::to_string(totalVolume);
	}
}
