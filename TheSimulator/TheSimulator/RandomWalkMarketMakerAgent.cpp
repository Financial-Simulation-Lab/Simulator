...
void RandomWalkMarketMakerAgent::receiveMessage(const MessagePtr& msg) {
    ...
    // std::cout << this->name() << ": Receive a Message" << std::endl;
...
// 		std::cout << this->name() << ":   Selling " << m_depth << " units " << "at price " << newSellPrice.toCentString() << ", then going to sleep to wait for the next order to be submitted" << std::endl;

		pptr = std::make_shared<PlaceOrderLimitPayload>(OrderDirection::Buy, m_depth, newBuyPrice);
		simulation()->dispatchMessage(currentTimestamp, 0, this->name(), m_exchange, "PLACE_ORDER_LIMIT", pptr);
		
// 		std::cout << this->name() << ":  Buying  " << m_depth << " units " << "at price " << newBuyPrice.toCentString() << ", then going to sleep to wait for the next order to be submitted" << std::endl;
