...
int main(int argc, char* argv[]) {
	...
	auto& silencio = cli.opt<bool>("s silent", false).desc("suppresses all verbose trace output, error traces remain enabled");
	...
	try {
		...
		try {
		    ...
		    std::cout << "Finish in " << time(0) - start_time << " seconds" << std::endl;
